...
namespace EWSSPullNotificationSample.Auth
{
    ...
    public class CredentialHandler
    {
        ...
        private EWSPullNotificationSample.Logger _logger = null;

        public CredentialHandler(AuthType authType, EWSPullNotificationSample.Logger Logger=null)
        {
            _authType = authType;
            _logger = Logger;
        }
        ...
        public void GetDelegatedToken()
        {
            try
            {
                _lastOAuthResult = Task.Run(async () => await OAuthHelper.GetDelegateToken(ApplicationId, TenantId)).Result;
                if (_lastOAuthResult != null)
                {
                    _logger?.Log($"Token obtained, expires {_lastOAuthResult.ExpiresOn}");
                    return;
        ...
        public void RenewToken()
        {
            try
            {
                _lastOAuthResult = Task.Run(async () => await OAuthHelper.RenewToken()).Result;
                if (_lastOAuthResult != null)
                {
                    _logger?.Log($"Token renewed, expires {_lastOAuthResult.ExpiresOn}");
                    return;
                }
            }
            catch (Exception ex)
            {
                MessageBox.Show(ex.Message, "Unable to renew token", MessageBoxButtons.OK, MessageBoxIcon.Error);
            }
        }
        ...
        public void LogCredentials(EWSPullNotificationSample.Logger Logger)
        {
